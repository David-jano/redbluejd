...
interface ArticleCommentsProps {
  articleId: string; // This is now the UUID from articles table
}
...
export default function ArticleComments({ articleId }: ArticleCommentsProps) {
  ...
  const fetchComments = async () => {
    if (!articleId) {
      setError("Cannot fetch comments: No article ID");
      setLoading(false);
      return;
    }
    ...
    try {
      ...
      // Fetch all comments from article_comments_new
      ...
      // Fetch replies for all comments from article_comment_replies_new
      const commentIds = commentsData.map((comment) => comment.id);
      let repliesData: any[] = [];

      if (commentIds.length > 0) {
        const { data, error: repliesError } = await supabase
          .from("article_comment_replies_new")
          .select("*")
      ...
      if (userName && userName.trim()) {
        const { data: likesData } = await supabase
          .from("article_comment_likes_new")
          .select("comment_id")
          .in("comment_id", commentIds)
  ...
  const handleAddComment = async () => {
    ...
    if (!articleId) {
      setError("Cannot post comment: No article ID");
    ...
    setError(null);
    try {
      const { error } = await supabase.from("article_comments_new").insert([
        {
  ...
  const handleLikeComment = async (commentId: number, isLiked: boolean) => {
    ...
    try {
      if (isLiked) {
        ...
        // Update the likes count in article_comments_new
