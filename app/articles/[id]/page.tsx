...
export default async function ArticleDetail({ params }: Props) {
  ...
  try {
    ...
    const [articleResult, commentCountResult, likeCountResult, moreArticlesResult] = await Promise.all([
      // Main article
      supabase
        .from("articles")
        .select("*")
    ...
    if (articleResult.error || !articleResult.data) {
      if (articleResult.error?.code !== "PGRST116") {
        logServerError("Fetching article", articleResult.error);
      }
      return notFound();
    }

    const article = articleResult.data;
    const commentCount = commentCountResult.error ? 0 : (commentCountResult.count || 0);
    const likeCount = likeCountResult.error ? 0 : (likeCountResult.count || 0);
    const moreArticles = moreArticlesResult.error ? [] : (moreArticlesResult.data || []);

    // Process image URLs
    const processedArticleImage = getValidImageUrl(article.image_url);
    const processedMoreArticles = moreArticles.map(article => ({
      ...article,
      image_url: getValidImageUrl(article.image_url)
    }));
    ...
    return (
      <div className="max-w-3xl mx-auto px-4 py-10">
        ...
        <section className="mt-16">
          ...
          {processedMoreArticles.length > 0 ? (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {processedMoreArticles.map((article) => (
                <article
                  key={article.id}
    ...
    console.error("Unexpected error in article page:", error);
    return notFound();
  }
}