...
export async function generateStaticParams() {
  const { data, error } = await supabase.from("header_cards").select("id");
...
export default async function HeaderCardDetailPage({ params }: Props) {
  ...
  if (error) {
    ...
    logServerError("Fetching header card", error);
    return notFound();
  }
  ...
  // Fetch like count using the new header_card_likes table
  ...
  try {
    ...
    logServerError("Fetching like count", error);
  }

  // Fetch other header cards for the "More Stories" section
  let moreCards: HeaderCard[] = [];
  try {
    const { data: moreCardsData, error: moreCardsError } = await supabase
      .from("header_cards")
      .select("*")
      .neq("id", cardId)
    ...
    if (moreCardsError) {
      logServerError("Fetching more header cards", moreCardsError);
    }

    if (moreCardsData) {
      moreCards = moreCardsData;
    }
  } catch (error) {
    logServerError("Unexpected error in more cards fetch", error);
  }
    ...
    <div className="max-w-3xl mx-auto px-4 py-10">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
        <div className="flex items-center flex-wrap gap-2 text-sm text-gray-600">
          <span>
            🖋️ <span className="font-medium">{card.author}</span>
          </span>
          <span>•</span>
          ...
          {card.is_featured && (
            <>
              ...
              <span className="flex items-center gap-1 text-black">
                <FaStar className="text-black" /> Featured
              </span>
            </>
          )}
        </div>

        {/* Header Card Like Button and Social Media Icons */}
      ...
      {/* Header Card Comments Section - Now with replies support */}
